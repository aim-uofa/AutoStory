...
parser.add_argument('--infer_thr', type=float, default=None, 
        help='threhshold for inference, following t2i-adaptor')
...
def test(test_loader, model, epoch, running_file, args):
    ...
    for idx, (image, img_name) in enumerate(test_loader):
        ...
        # NOTE optionally apply thresholding here
        if args.infer_thr is not None:
            result_dtype = result.dtype
            result = (result > 0.5).astype(result_dtype)
...
if __name__ == '__main__':
    data_dir_share = args.datadir
    savedir_share = args.savedir

    for datadir in glob.glob(os.path.join(data_dir_share, '*', '*', '*')):  # iters/prompt/image/
        args.datadir = datadir
        args.savedir = args.datadir.replace(data_dir_share, savedir_share)
