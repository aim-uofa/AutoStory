...
def register_region_aware_attention(model, layerwise_embedding=False):
    ...
    # def reshape_to_video(x):
    #     video_length = x.shape[0] // 2
    #     negative_part = x[0:video_length, ...]  # (bs, h*w, dim_per_head*head)
    #     positive_part = x[video_length:, ...]
    #     video = torch.stack([negative_part, positive_part], dim=0)  # (2, f, h*w, dim_per_head*head)
    ...
    def get_new_forward(cross_attention_idx):
        ...
        def new_forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, **cross_attention_kwargs):
            ...
            query = self.to_q(hidden_states)  # (bs, h*w, dim_per_head*head)
            ...
            if not is_cross:
                ...
                # negative propmt first, then positive prompt
                # the first half are negative prompts, the second half are positive prompts

                video_length = batch_size // 2
            ...
            query = self.head_to_batch_dim(query)  # (bs * head, h * w, dim_per_head)
            key = self.head_to_batch_dim(key)  # (bs * head, h * w, dim_per_head)
            value = self.head_to_batch_dim(value)  # (bs * head, h * w, dim_per_head)

            attention_probs = self.get_attention_scores(query, key, attention_mask)  # (bs * head, h * w, h * w)
    ...
    def change_forward(unet, cross_attention_idx):  # omit proceesor in new diffusers
        for name, layer in unet.named_children():
            if layer.__class__.__name__ == 'CrossAttention':
                ...
                if name == 'attn1':  # we handle self-attn
...
def StableDiffusion_Sample(
    ...
    register_region_aware_attention(self)
