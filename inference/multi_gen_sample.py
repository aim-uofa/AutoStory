...
from multi_gen_sample_util import StableDiffusion_PPlus_Sample, StableDiffusion_Sample
...
def inference_image(pipe,   
    ...
    if pipeline_type == 'adaptor_pplus' or pipeline_type == 'adaptor_sd':
        ...
        new_concept_cfg = extra_kargs.pop('new_concept_cfg')
        width = extra_kargs.pop('width')
        height = extra_kargs.pop('height')
    ...
    elif pipeline_type == 'sd' or pipeline_type == 'pure_sd':
        ...
        sketch_condition = extra_kargs.pop('sketch_condition')
        if sketch_condition is not None:
            sketch_adapter_input = sketch_condition  # NOTE we use different sketch for each batch
        else:
            sketch_adapter_input = None
...
def merge_pplus2sd_(pipe, lora_weight_path):
    ...
    lora_weight = torch.load(lora_weight_path, map_location='cuda')['params']
    new_concept_cfg = {}
    ...
    if 'text_encoder' in lora_weight and len(lora_weight['text_encoder']) != 0:
        ...
        for k in lora_weight['text_encoder'].keys():
            ...
            # if 'down' in k:
            #     sd_textenc_state_dict[k.replace('_lora.down', '')] += 0.8 * (lora_weight['text_encoder'][k.replace('down', 'up')] @ lora_weight['text_encoder'][k])
    ...
    if 'unet' in lora_weight and len(lora_weight['unet']) != 0:
        ...
        for k in lora_weight['unet'].keys():
            ...
            # if '.down' in k:
            #     sd_unet_state_dict[k.replace('_lora.down', '')] += 0.8 * (lora_weight['unet'][k.replace('.down', '.up')] @ lora_weight['unet'][k])

        unet.load_state_dict(sd_unet_state_dict)
    return new_concept_cfg

def merge2sd_lora_(pipe, lora_weight_path):
    ...
    if 'text_encoder' in lora_weight and len(lora_weight['text_encoder']) != 0:
        ...
        for k in lora_weight['text_encoder'].keys():
            if 'down' in k:
                sd_textenc_state_dict[k.replace('_lora.down', '')] += 1 * (lora_weight['text_encoder'][k.replace('down', 'up')] @ lora_weight['text_encoder'][k])
        text_encoder.load_state_dict(sd_textenc_state_dict)
    ...
    if 'unet' in lora_weight and len(lora_weight['unet']) != 0:
        ...
        for k in lora_weight['unet'].keys():
            if '.down' in k:
                sd_unet_state_dict[k.replace('_lora.down', '')] += 1 * (lora_weight['unet'][k.replace('.down', '.up')] @ lora_weight['unet'][k])
...
def build_model(pretrained_model, combined_model, sketch_adaptor_model, keypose_adaptor_model, pipeline_type, device):
    ...
    if pipeline_type == 'adaptor_pplus' or pipeline_type == 'adaptor_sd':
        ...
        elif pipeline_type == 'adaptor_sd':
            merge2sd_lora_(pipe, lora_weight_path=combined_model)
            new_concept_cfg = {}
        else:
...
def parse_args():
    ...
    parser.add_argument('--pretrained_model', default='/mnt/nas/share/home/canyu/Diffusion/checkpoints_diffusers/chilloutmix', type=str)
    parser.add_argument(
        '--combined_model',
        default='/mnt/nas/share/home/canyu/Diffusion/Mix-of-Show/scripts/mixofshow_scripts/paper_result_scripts/combined_model.pth',
        type=str)
    parser.add_argument('--sketch_adaptor_model', default=None, type=str)
    parser.add_argument('--sketch_condition', default=None, type=list)
    ...
    parser.add_argument('--save_dir', default='./results/sampling/', type=str)
    parser.add_argument('--pipeline_type', default='adaptor_sd', type=str)
    parser.add_argument('--prompt', default='photo of a <dogB> and a <catA>', type=list)
    parser.add_argument('--negative_prompt', default='sketches, NSFW,deformed,large breasts,missing limbs,amputated,pants,shorts,cat ears,bad anatomy, naked, no clothes,disfigured, poorly drawn face, mutation, mutated,ugly, disgusting, blurry, watermark, watermarked, oversaturated, obese, doubled face,b&w, black and white, sepia, nude, frekles, no masks,duplicate image, blur,', type=str)
    parser.add_argument('--prompt_rewrite', default='', type=str)
    parser.add_argument('--seed', default=None, type=int)
    parser.add_argument('--suffix', default='', type=str)
    parser.add_argument('--prefix', default='', type=str)  # prefix makes results cleaner
    parser.add_argument('--num_samples_per_panel', default=1, type=int)
...
def main(args):
    ...
    if args.pipeline_type == 'adaptor_pplus' or args.pipeline_type == 'adaptor_sd':
        if args.sketch_condition is not None and os.path.exists(args.sketch_condition):
            ...
            # sketch_condition = sketch_condition.resize((1024, 512))
    ...
    elif args.pipeline_type == 'sd' or args.pipeline_type == 'pure_sd':

        if args.sketch_condition is not None:
            sketch_condition = []
            print(args.sketch_condition)
            for sketch_condition_tmp in args.sketch_condition:
                sketch_condition_tmp = Image.open(sketch_condition_tmp).convert('L')
                sketch_condition.append(sketch_condition_tmp)
                # sketch_condition = sketch_condition.resize((1024, 512))
                width_sketch, height_sketch = sketch_condition_tmp.size
                print('use sketch condition')
        else:
            sketch_condition, width_sketch, height_sketch = None, 0, 0
            print('skip sketch condition')
        ...
        #     'width': 512,
        # }
        input_prompt = list(args.prompt)
        save_prompt = input_prompt[0]
    elif args.pipeline_type == 'sd_pplus':
        kwargs = {
            'new_concept_cfg': new_concept_cfg,
            'height': 512,
    ...
    # hash_code = torch.rand(1)[0]
    
    save_prompt = save_prompt.replace(' ', '_')
    save_name = f'{args.prefix}---seed{args.seed}---{save_prompt}---{args.suffix}.png'  # exclude hash code

    save_dir = os.path.join(args.save_dir, f'rst_{args.pipeline_type}')
    save_config_path = os.path.join(save_dir, save_name.replace('.png', '.txt'))

    os.makedirs(save_dir, exist_ok=True)
    for idx, img in enumerate(image):
        img.save(os.path.join(save_dir, f'{idx}_seed{args.seed}.png'))
...
if __name__ == '__main__':
    ...
    num_samples_per_panel = args.num_samples_per_panel
    if args.seed is not None:
        assert num_samples_per_panel == 1, 'seed is not None, but num_samples_per_panel > 1'

    for repeat_idx in range(num_samples_per_panel):

        args.seed = torch.randint(0, 10000, (1,)).item()

        # for panel_idx in range(config.num_pannels):
        if 'keypose_adaptor_weight' in config:
            args.keypose_adaptor_weight = config.keypose_adaptor_weight[0]
        if 'sketch_adaptor_weight' in config:
            args.sketch_adaptor_weight = config.sketch_adaptor_weight[0]
        if 'sketch_condition' in config:
            args.sketch_condition = config.sketch_condition
        if 'keypose_condition' in config:
            args.keypose_condition = config.keypose_condition[0]

        # args.prompt = config.background_prompt[panel_idx]  # use background_prompt as context prompt, not global_prompt
        args.prompt = config.global_prompt
        # args.prompt_rewrite = config.prompt_rewrite[panel_idx]
        args.negative_prompt = config.negative_prompt[0]
        # args.prefix = f"panel{panel_idx}"
