...
def parse_args():
    ...
    parser.add_argument('--prompt', default='photo of a <dogB> and a <catA>', type=str)
    ...
    for repeat_idx in range(num_samples_per_panel):
        ...
        for panel_idx in range(config.num_pannels):
            if 'keypose_adaptor_weight' in config:
                args.keypose_adaptor_weight = config.keypose_adaptor_weight[panel_idx]
            if 'sketch_adaptor_weight' in config:
                args.sketch_adaptor_weight = config.sketch_adaptor_weight[panel_idx]
            if 'sketch_condition' in config:
                args.sketch_condition = config.sketch_condition[panel_idx]
            if 'keypose_condition' in config:
                args.keypose_condition = config.keypose_condition[panel_idx]
            ...
            args.negative_prompt = config.negative_prompt[panel_idx]
